                                                    ...
                                                    int64_t tstamp_threshold, thread_id_t tid)
{
    ...
    while (unset_mark((long)left_node->next.load(std::memory_order_seq_cst)))
    {
        ...
        if (left_node_vhead->value != -1)
        {
            ...
            while(unset_freeze((long)left_vnode->next.load(std::memory_order_seq_cst))) {
                curr_vnode = unset_freeze((long)left_vnode->next.load(std::memory_order_seq_cst));
                if(curr_vnode->ts <= tstamp_threshold) {
                    left_vnode->next = nullptr;
...
template <typename K, typename V>
bool Linked_List<K, V>::search(K key, V value)
